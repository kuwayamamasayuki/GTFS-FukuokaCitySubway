...
- 件数: **198 出入口**。対象は当時の**全 35 駅（`parent_station` 1〜35）**。
...
2023 年開業の **櫛田神社前（id 36）** と **七隈線博多（id 37）** は旧フィード（2018年版）には
存在せず、出入口は未収録である。なお博多複合駅の出入口は旧フィードでは当時の **id 11
（博多）配下**にまとまっており、本実装でも 11 配下に入る。
新駅の出入口を追加する場合は、実測座標を `scripts/data/legacy_entrances.csv` に
追記すればよい（`parent_station` に 36 / 37 を指定）。
